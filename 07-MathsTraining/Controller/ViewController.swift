...
//  07-MathsTraining
//
//  Created by Ricardo Sanchez on 7/9/20.
...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func numberDraw(_ image: UIImage) {
        ...
        //1- Redimencionaremos la imagen del usarioa a una 299 * 299 ( tamaño que espera el modelo )
        let modelSize = 299
        ...
        guard let model = try? VNCoreMLModel(for: mnisTModel.model) else {
            fatalError("No se ha podido preparar el modelo de clasificacion para Vision")
        }
                
        
        //4- Peticion VNCoreMLRequest
        let request = VNCoreMLRequest(model: model) { [weak self] (request, error) in
           //5- AL detectar la imagen tenemos que saber el numero que ha escrito el usuario y validar la respuesta
            
            guard let results = request.results as? [VNClassificationObservation],
                let prediction = results.first, let prediction2 = results.last else{
                    fatalError("Error al hacer la prediccion: \(error?.localizedDescription ?? "Error Desconocido")")
            }
            DispatchQueue.main.async {
                //El reultado es la etiqueta dela clase donde el modelo ha catalogado nuestra imagen
                let result = Int(prediction.identifier) ?? 0
                let result2 = Int(prediction2.identifier) ?? 0
                //Asignamos la respuesta a la pregunta actual
                self?.factory.compareAnswers(at: 0, with: result, with: result2)
                //Actualizar la celda
        ...
        //Ejecutamos la prediccion en un hilo secundario para no bloquear la UI
    ...
    func askNextQuestion() {
        ...
        if(timeLeft > 0){
            ...
            if let cell = tableView.cellForRow(at: secondIndexPath), let qustion = factory.getQuestionAt(position: 1){
                setText(for: cell, at: secondIndexPath, to: qustion)
            }
        } else {
            gameTimer.invalidate()
            
            let controller = UIAlertController(title: "Fin de la partida", message: "Has conseguido \(self.factory.score) puntos/ \(self.factory.getNumberOfQuestion() * self.factory.pointsPerQuestion)", preferredStyle: .alert)
            
            let action = UIAlertAction(title: "Jusgar de nuevo", style: .default, handler: restartGame(action:))
